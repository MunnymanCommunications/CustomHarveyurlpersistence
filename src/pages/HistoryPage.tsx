  ...
  return (
    <div className="glassmorphic rounded-2xl shadow-2xl p-4 sm:p-8 max-w-4xl mx-auto w-full h-full flex flex-col">
      ...
      <div className="flex-grow space-y-4 mb-6 overflow-y-auto pr-2">
        {history.length === 0 ? (
          <p className="text-text-secondary text-center py-8">No history yet.</p>
        ) : (
          history.map((entry, index) => (
            <div key={index} className="p-4 bg-white/60 rounded-lg border border-border-color">
              <p className="font-semibold text-brand-secondary-glow">You:</p>
              <p className="mb-2 text-text-secondary">{entry.user || "..."}</p>
              <p className="font-semibold text-brand-tertiary-glow">Assistant:</p>
              <p className="text-text-secondary">{entry.assistant || "..."}</p>
              <p className="text-xs text-right text-text-tertiary mt-2">{new Date(entry.timestamp).toLocaleString()}</p>
            </div>
          ))
        )}
      </div>
