...
export default function Orb({ hue = 0, hoverIntensity = 0.2, rotateOnHover = true, forceHoverState = false }: OrbProps) {
  ...
  useEffect(() => {
    ...
    try {
        ...
        // FIX: Cast canvas to HTMLCanvasElement as it's being appended to the DOM.
        container.appendChild(gl.canvas as HTMLCanvasElement);
    } catch (e) {
        console.error("Failed to initialize WebGL for Orb component:", e);
        return; // Exit if WebGL is not supported, preventing a crash.
    }

    // FIX: Cast WebGLRenderingContext to `any` to match the expected OGLRenderingContext type.
    ...
    function resize() {
      ...
      // FIX: Cast canvas to HTMLCanvasElement to access the style property.
    ...
    return () => {
      ...
      if (container && gl.canvas) {
        try {
            // FIX: Cast canvas to HTMLCanvasElement to remove it from the DOM.
