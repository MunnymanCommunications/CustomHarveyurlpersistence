...
const NavItem: React.FC<{
  ...
  <li>
    ...
    >
      <div className={`p-2 rounded-lg transition-all duration-300 ${isActive ? 'bg-gradient-to-br from-brand-secondary-glow to-brand-tertiary-glow text-on-brand shadow-md' : 'bg-base-light text-text-secondary group-hover:bg-white'}`}>
      ...
      <nav className={`fixed md:relative top-0 left-0 h-full z-50 bg-gradient-to-b from-white/80 to-white/60 backdrop-blur-xl border-r border-border-color p-4 flex flex-col transition-all duration-300 ease-in-out 
        ...
        <header className="flex items-center gap-3 p-2 mb-8 relative">
          ...
          <div className={`flex flex-col overflow-hidden transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0' : 'w-full opacity-100'}`}>
            <h1 className="text-xl font-bold text-text-primary whitespace-nowrap">{assistantName}</h1>
            <p className="text-sm text-text-secondary whitespace-nowrap">Personal Assistant</p>
          </div>
          <button onClick={onMobileClose} className="md:hidden absolute top-0 right-0 p-2 text-text-secondary hover:text-text-primary">
        ...
        <div className="pt-4 border-t border-border-color/50">
          <button 
              ...
              className="hidden md:flex items-center w-full p-3 rounded-lg text-text-secondary hover:bg-white/70 hover:text-text-primary justify-center"
              aria-label={isCollapsed ? "Expand sidebar" : "Collapse sidebar"}
          >
              <Icon name={isCollapsed ? 'chevronRight' : 'chevronLeft'} className="w-5 h-5" />
          </button>
          <div className={`text-center text-xs text-text-tertiary pt-4 transition-all duration-300 overflow-hidden ${isCollapsed ? 'h-0 opacity-0' : 'h-auto opacity-100'}`}>
